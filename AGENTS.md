...
> 本文件只保留 Codex 在本仓库每次都必须遵守的硬规则。
> 项目背景、阶段进度、模块说明优先结合 `README.md` 与 `docs/` 阅读。
> 详细示例与参考模板按需查看 `docs/22-Agent参考规范与示例.md`。
...
- 每次进入仓库后，先阅读以下文件，再开始分析代码或执行任务：
  1. `README.md`
  2. `docs/21-当前项目状态与接手指南.md`
  3. `docs/08-开发顺序与落地清单.md`
  4. `docs/06-工程演进与当前阶段判断.md`
- 开始编码、改文件、给方案前，先总结一次当前进度，至少说明：
  - 当前阶段
  - 已完成的关键能力
  - 当前风险和边界
  - 下一步最高优先级
- 当前项目进度默认以 `docs/21-当前项目状态与接手指南.md` 作为单页入口。
- 需要确认实际落地状态时，补充查看：
  - `git status --short`
  - 最近一次关键提交
  - 对应模块源码与装配代码
...
- 提交信息必须使用英文。
- 提交标题格式固定为 `Type: Subject`。
- `Type` 只能使用：`Feat` / `Fix` / `Refactor` / `Perf` / `Style` / `Docs` / `Revert` / `Build`。
- `Subject` 使用简洁祈使句或名词短语，首字母大写，不以句号结尾。
