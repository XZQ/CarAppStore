...
object InstallerText {
    ...
    const val FAILURE_APK_PACKAGE_MISMATCH = "安装包包名与目录不一致"
    const val FAILURE_APK_VERSION_MISSING = "目录缺少安装包版本代码"
    const val FAILURE_APK_VERSION_MISMATCH = "安装包版本与目录不一致"
    const val FAILURE_APK_SIGNER_MISSING = "目录缺少安装包签名证书摘要"
    const val FAILURE_APK_SIGNER_MISMATCH = "安装包签名证书不受信任"
    const val FAILURE_INSTALLED_PACKAGE_MISMATCH = "系统安装结果包名不一致"
    const val FAILURE_INSTALLED_PACKAGE_NOT_FOUND = "系统未找到已安装应用"
    const val FAILURE_INSTALLED_VERSION_MISMATCH = "系统安装结果版本不一致"
