...
import com.xzq.appstore.feature.myapp.databinding.ItemMyAppBinding
...
class MyAppAdapter(
    ...
    class MyAppViewHolder(
        ...
        fun bind(item: AppViewData) {
            binding.tvAppInitial.text = item.iconText.ifBlank { item.name.firstDisplayChar() }
            AppImageLoader.load(binding.ivAppIcon, item.iconUrl, binding.tvAppInitial)
        ...
        /** 提取应用名称首个可见字符作为轻量图标。 */
        private fun String.firstDisplayChar(): String = trim().firstOrNull()?.toString().orEmpty()
    }
