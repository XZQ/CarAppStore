...
> 最后更新：2026-07-31
> 本文档面向后端、客户端、设备厂商和可选 OEM 团队，说明 CarAppStore Android 客户端的分层验收标准。后端和 Android 设备基线为通用门禁；OEM 车况只适用于车载发行目标。
...
- [ ] **CDN 认证模式明确**：`RUNTIME` 模式的 HEAD/GET/Range 请求读取短期凭证且端点不依赖重定向；验证签发、刷新、过期及 401/403；`SIGNED_URL` 模式使用目录下发的短时签名 HTTPS URL（见 docs/35）

## 2. 可选车载 OEM 信号验收标准

> 本节只有在发行目标声明支持车载设备时执行。非车载目标应标记 N/A；当前平台能力开关已确保缺少车况信号不会阻断安装或升级。
...
| `StaticVehicleStateSignalProvider`  | 当前历史兜底实现（固定输出驻车=false）；仅适合作为车载 fail-closed 语义，不应成为通用平台默认门禁 |
...
### 3.2 Android 版本/ROM 差异验收

不同 Android 版本、设备厂商和 ROM 可能在以下方面存在差异，需要逐设备记录；车载发行目标可在同一模板中追加车型和 OEM 专项字段：

```
| 项目                     | 手机A（ROM 1.0） | 平板B（ROM 2.0） | 大屏C（ROM 1.5） |
|-------------------------|------------------|------------------|------------------|
| 系统确认页样式            |                  |                  |                  |
| 确认页是否可自定义        |                  |                  |                  |
| 安装成功回调码            |                  |                  |                  |
...
1. 在每款目标 Android 设备上安装 CarAppStore APK
...
| 第2周 | 第 6-10 天| 平台策略能力开关与 Android 兼容性联调             | 非车载目标不依赖车况，设备差异可追踪 |
| 第3周 | 第 11-15 天| Android 设备矩阵全链路验证；按需并行 OEM 专项      | 下载-安装-升级通过，车载项可 PASS/N/A |
...
- [ ] 非车载发行目标未启用车况能力时，安装和升级不依赖 OEM 信号
- [ ] 车载能力开关、默认值和回退策略有自动化测试
...
| 可选 OEM 信号延迟或丢失 | OEM/设备厂商 | 车载专项中延迟 > 500ms 或信号中断 | 联调群 + 工单 |
| 安装器 ROM 差异    | 客户端/设备厂商 | 回调码或确认页行为与预期不符 | 联调群 + 工单    |
