...
| **R2** 真实安装 | PackageInstaller.Session、用户确认流、Session持久化 | `RealPackageInstaller.kt` | 2 tests |
| **D** 统一数据层 | LocalStoreFacade、JsonBackedLocalStoreFacade、实体映射 | `data/local/` | mapper/facade tests |
| **R3** 执行控制 | IO中断(registerInterrupt)、网络异常处理、去重保护 | `DownloadExecutionControl.kt` | — |
| **R4** Repository真实化 | RealAppRepository、AppSystemDataSource接入PackageManager | `RealAppRepository.kt` | repository/system datasource tests |
...
- 安装链路需要在 Android 版本/ROM 设备矩阵上联调差异；车载 OEM 只作为可选专项
- 远端目录已有 HTTP/缓存/raw 资源回退链路，但默认配置仍是示例地址，未接真实生产 API
- `DefaultPolicyCenter` 已接网络/存储实时信号，并通过能力开关隔离可选 OEM 驻车限制；目录和业务动作已接入 `supportedPlatforms/currentPlatform`，车载发行才需要接真实车机协议
